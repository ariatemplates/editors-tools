...
### Confusion between `_index` and `_parentIndex` in `Node`

__Fix the confusion between the two indexes, which don't follow their semantics.__

`_parentIndex` should be the index among all its children - what is `_index` for now -instead of the one inside the list it is in (if so).
...
This way there can be a lazy updates of the graph.
