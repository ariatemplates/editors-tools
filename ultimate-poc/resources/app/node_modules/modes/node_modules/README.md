...
* `highlight`: module for highlighting
* `outline`: module for outline

Models:

* `mode.js`: interface to all the services a mode can provide
* `node.js`: a node is a part of a graph, and in this case it used for one of the representations of a source code
* `code.js`: provides various models/representations and operations related to a source code
...
* Fix the confusion between `_index` and `_parentIndex`: `_parentIndex` should be the index among all its children (what is `_index` for now), not the one inside the list it is in (if so)

### TODO

* be able to compute missing things, like end locations (this issue occurs for PEG.js based parsers)
* use properties definitions (like proxies): this way there can be a lazy update. When the user of the node requests a property value, the node can check if there are things to re-compute (basically its subgraph changed), does it, caches the value, and anyway returns this value. The little overhead of using the getter anytime is worth it I guess.
...
Please refer to the `highlight` submodule.
...
Every standard mode has the same kind of elements. Stick with data oriented design, and instead of making them repeating things (how to compile a grammar, how to test the parser, how to test the mode), let them provide data and process it.

### Testing

* Add a testing library to test/try modes
