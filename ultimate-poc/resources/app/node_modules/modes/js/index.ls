...
class JS extends Mode
	...
	_getSessionData = (guid) -> @@sessions[guid] ? @_init guid

	_updateSession: ({guid}:arg) ->
		data = @_getSessionData guid

		@_updateModels data, arg

	_updateModels = (data, {source, index}) ->
		index ?= 0

		# source ---------------------------------------------------------------

		{oldSource} = data.models


		data.models.source = (data.models.source.substring 0, index) + source + (data.models.source.substring index + source.length, data.models.source.length)
