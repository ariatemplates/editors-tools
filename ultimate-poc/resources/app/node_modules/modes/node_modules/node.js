...
var Position = oop.Class({
	...
	schema: {
		...
		properties: [
			{names: ['index', 'offset', 'pos', 'position', 'char', 'character']},
			{names: ['line', 'l']},
			{names: ['column', 'c']}
		]
	}
...
var Node = oop.Class({
	...
	schema: {
		properties: [
			{names: ['type'], ctor: NodeType, mixed: true},
			{names: ['location', 'loc'], ctor: Location, mixed: true},
	...
	init: function() {
		...
		// Maybe this should be moved or more generally refactored in the future
	...
	methods: {
		...
		/**
		 * Generic function to add something to the node: a child, a list of child, or a property.
		 */
		put: function(id, item) {
			if (item instanceof Node) {
				thid.add(id, item);
			} else if (prelude.isType('Array', item) && item.length > 0 && item[0] instanceof Node) {
				this.addList(id, item);
			} else {
				this.set(id, item);
			}
		},
		...
		getId: function() {
			return {
				...
				range: [this.location.start.index, this.location.end.index]
			}
		},
		...
		getPath: function() {
			...
			while (current.parent != null) {
				if (current._parentIndex != null) {
					fragments.push(current._parentIndex);
				}
				fragments.push(current._parentId);
		...
		getPosition: function() {
			if (this.parent != null) {
				return this.parent.getPosition() + '.' + this._index;
			} else {
				return '0';
			}
		},
		...
		simplify: function() {
			...
			var start = this.location.start.index;
			var end = this.location.end.index;

			// loc = "L#line C#column (#start)"---
			var loc = line + "x" + column + "/" + start + "->" + end;
