...
var Mode = oop.Class({
	...
	schema: {
		properties: [
			...
			{names: ['_stylesheet', 'stylesheet'], type: Stylesheet},
	...
	prototype: {
		...
		html: function(code, input) {
			...
			for (var i = 0, length = ranges.length; i < length; i++) {
				...
				var str;
				if (range.style === "eols") {
					str = prelude.Str.repeat(source.substring(range.start, range.end).replace('\r\n', '\n').length, '<br/>');
				} else if (range.style === "spaces") {
					str = prelude.Str.repeat(range.end - range.start, '&nbsp;');
				} else if (range.style === "tabs") {
					str = prelude.Str.repeat((range.end - range.start) * 4, '&nbsp;');
				} else {
					str = escapeHTML(source.substring(range.start, range.end));
				}
				var style = '';
				if (range.style != '') {
		...
		outline: function(code, input) {
			var type = dop.extractProperty(input, {names: ['type']});
