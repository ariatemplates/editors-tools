...
override = (original, newValues = {}) ->
	for key, value of newValues => if original[key]? => original[key] = value
	original
 ...
 */
factory = (value, key = value) -> if type.isObject value => value else {(key): value}
...
transform = (obj, cb, rec = no) ->
	if not rec => cb obj
		...
		{[key, cb[key](value)] for key, value of obj}
