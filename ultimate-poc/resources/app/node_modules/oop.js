	...
	var constructor = function() {
		...
		prelude.each(function(property) {
			prelude.each(function(name) {
				Object.defineProperty(this, name, property.descriptor);
			}, property.names);
		}, properties);
	}
 ...
 * @see applyProperties
 */
function processProperties(output, input, rules) {
	var inputToSpecRules = rules.inputToSpec;
	var propertiesSpecs = rules.properties;

	var spec = inputToSpec(input, inputToSpecRules);
	// output == null && (output = {});
	return applyProperties(output, spec, propertiesSpecs);
}
 ...
 * @todo do some refactoring to be able to apply a single property to the output, keepng a function able to return it only. This way, the function altering the output will use the other one, and will take care of the real output name (see previous todo point)
 */
 ...
 *
 * @fixme When the factory is applied, if no suitable property value was found, teh spec itself is used as input. For instance, this way specifying {color: {r, g, b}, font} will work as well as {r, g, b, font} directly. HOWEVER this brings a problem when you want to actually bypass a value, set is as undefined. Take the second object, and imagine {r, g, b} were user-specific extra properties, not the spec for the color. Or imagine there is nothing at all for the color ({font}), when trying to build the color property, it should fail. THE USER SHOULD SPECIFY MORE RULES TO HANDLE THIS PROPERLY.
 */
