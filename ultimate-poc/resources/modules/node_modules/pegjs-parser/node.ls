...
class Node
	 ...
	 */
	(language, element, line, column, index) ~> @ <<< {
		type: {
			language
			element
		}
	 ...
	 */
	add: (id, child) ->
		@_pushChild @[]children, id, child

		@{}childrenIndex[id] = child

	_pushChild: (children, id, child) -> child
	...
	/**
	 * DISCLAIMER Might not work in all cases, especially with empty arrays
	 *
	...
	traverse: (cb) ->
		cb @
		for child in @children => @traverse child, cb
	...
	simplify: ->
		...
		{}
			...
			..props? = @properties
			..children? = if not @children? => void else [child.simplify! for child in @children ? []]
